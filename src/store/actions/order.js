...
export const fetchOrders = () => {
	return async (dispatch, getState) => {
		...
		try {
			const response = await fetch(`https://rn-guide-shop.firebaseio.com/orders/${userId}.json`)
...
export const addOrder = (cartItems, totalAmount) => {
	return async (dispatch, getState) => {
		...
		const userId = getState().auth.userId
		const date = new Date()
		const response = await fetch(`https://rn-guide-shop.firebaseio.com/orders/${userId}.json?auth=${token}`, {
			method: 'POST',
			headers: {
